...
class Model():
    ...
    def train(self, train_input, train_target, num_epochs) -> None:
        ...
        tarin_pair = torch.stack((train_input, train_target), dim=1) # Create (input, target) pair (N, 2, C, H, W)
        ...
        if self.tb:
            ...
            writer.add_graph(self.unet, input_to_model=torch.randn((1, 3, 32, 32)).to(self.device))
        idx_num = 0
        for epoch in range(num_epochs):
            ...
            for idx, img_pairs in enumerate(train_loader):
                ...
                idx_num = idx_num + 1
                if idx % 100 == 99:
                    print('Epoch %d, Batch %d >>>>>>>>>>>> Loss: %.3f, PSNR: %.3f dB' % (epoch, idx + 1, running_loss / 100, running_psnr / 100))
                    if self.tb:
